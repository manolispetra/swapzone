...
export default function Navbar() {
  ...
  const { shortAddress, connect, disconnect, connecting, isConnected } = useWallet();
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(({ href, label }) => (
            <Link key={href} href={href}
              ...
              style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", fontSize: 13 }}>
              {label}
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3">
          {isConnected ? (
            <button onClick={disconnect}
              className="flex items-center gap-2 px-4 py-2 border border-primary/30 rounded-xl text-primary text-sm hover:bg-primary/5 transition-all"
              style={{ fontFamily: "'Space Mono', monospace", fontSize: 12 }}>
              <span className="w-2 h-2 rounded-full bg-primary animate-pulse-slow" />
              {shortAddress}
            </button>
          ) : (
            <button onClick={connect} disabled={connecting} className="btn-primary flex items-center gap-2" style={{ padding: "8px 18px", fontSize: 13 }}>
              <Wallet size={14} />
        ...
        <div className="md:hidden border-t border-border bg-bg/95">
          {NAV_LINKS.map(({ href, label }) => (
            <Link key={href} href={href} onClick={() => setMobileOpen(false)}
              className={`block px-5 py-3 text-sm border-b border-border/50 no-underline transition-colors ${
